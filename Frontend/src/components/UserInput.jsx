...
const UserInput = ({ onSendMessage, input, setInput, isLoading }) => {
  ...
  return (
    <form
      ...
      className="px-4 py-3 border-t border-gray-800 bg-gradient-to-t from-transparent to-black"
        ...
        <div className="flex items-center">
          <button
            ...
            className={`inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-medium transition
              ${isLoading || !input.trim() ? 'bg-indigo-600/40 cursor-not-allowed text-gray-200' : 'bg-indigo-600 hover:bg-indigo-700 text-white'}`}
          >
            {isLoading ? (
              <svg className="w-4 h-4 animate-spin text-white align-middle" viewBox="0 0 24 24" fill="none">
